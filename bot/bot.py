...
def check_proxy(p_data):
    if len(p_data) == 3:
        srv, prt, sec = p_data
        url = f"https://t.me/proxy?server={srv}&port={prt}&secret={sec}"
    else:
        srv, prt = p_data
        url = f"https://t.me/proxy?server={srv}&port={prt}&secret=ee00000000000000000000000000000000676f6f676c652e636f6d"
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            .btn { background: #38bdf8; color: black; text-decoration: none; padding: 10px 20px; border-radius: 8px; font-weight: bold; display: inline-block; }
        </style>
    </head>
    <body>
        <h1>🛰 PROXY WEB</h1>
...
def start_cmd(m):
    users.add(m.chat.id)
    bot.send_message(m.chat.id, "🦾 **Бот готов к работе!**\n\n/get — Список прокси\n/admin — Статистика")

@bot.message_handler(commands=['get'])
def get_cmd(m):
    users.add(m.chat.id)
    wait = bot.send_message(m.chat.id, "🛰 **Ищу самые быстрые прокси...**")
    valid = get_fresh_proxies(6)
    if valid:
        res = "📡 **НАЖМИ НА ССЫЛКУ ДЛЯ ПОДКЛЮЧЕНИЯ:**\n\n"
        for p in valid:
            # Здесь убрали кавычки, чтобы ссылка была кликабельной
            res += f"{p['icon']} **{p['ms']}ms**\n{p['url']}\n\n"
        bot.edit_message_text(res, m.chat.id, wait.message_id, parse_mode="Markdown", disable_web_page_preview=True)
    else:
        bot.edit_message_text("❌ Все прокси пока спят. Попробуй через 30 сек!", m.chat.id, wait.message_id)
